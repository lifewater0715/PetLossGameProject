...
public class Book : MonoBehaviour
{
    ...
    private Coroutine leftAnimationCoroutine;
    private Coroutine rightAnimationCoroutine;
    private Coroutine leftNextAnimationCoroutine;
    private Coroutine rightNextAnimationCoroutine;

    private int leftPageIndex = -999;
    ...
    private Vector3 Calc_C_Position(Vector3 followLocation)
    {
        ...
        Vector3 r1 = new Vector3(
            radius1 * Mathf.Cos(F_SB_Angle),
            radius1 * Mathf.Sin(F_SB_Angle),
            0) + sb;
        ...
        float C_ST_dy = result.y - st.y;
        float C_ST_dx = result.x - st.x;
        float C_ST_Angle = Mathf.Atan2(C_ST_dy, C_ST_dx);

        Vector3 r2 = new Vector3(
            radius2 * Mathf.Cos(C_ST_Angle),
            radius2 * Mathf.Sin(C_ST_Angle),
            0) + st;

        float C_ST_distance = Vector2.Distance(result, st);
    ...
    public void DragRightPageToPoint(Vector3 point)
    {
        ...
        SetAnimatedPage(Left, currentPage, ref leftAnimationCoroutine, ref leftPageIndex);

        Right.gameObject.SetActive(true);
        Right.transform.position = RightNext.transform.position;
        Right.transform.eulerAngles = Vector3.zero;

        SetAnimatedPage(Right, currentPage + 1, ref rightAnimationCoroutine, ref rightPageIndex);
        SetAnimatedPage(RightNext, currentPage + 2, ref rightNextAnimationCoroutine, ref rightNextPageIndex);
    ...
    public void DragLeftPageToPoint(Vector3 point)
    {
        ...
        SetAnimatedPage(Left, currentPage - 2, ref leftAnimationCoroutine, ref leftPageIndex);
        SetAnimatedPage(LeftNext, currentPage - 3, ref leftNextAnimationCoroutine, ref leftNextPageIndex);
    ...
    {
        int steps = Mathf.Max(1, (int)(duration / 0.025f));
    ...
    private void SetAnimatedPage(Image target, int pageIndex, ref Coroutine animationCoroutine, ref int cachedPageIndex)
    {
    ...
    private void StopPageAnimation(ref Coroutine coroutine)
    {
